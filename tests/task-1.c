...
test_cancel_creation (void)
{
	...
	g_assert (iris_task_is_cancelled (task) == FALSE);
	iris_task_cancel (task);
	g_assert (iris_task_is_cancelled (task) == TRUE);
...
test22 (void)
{
	...
	iris_task_cancel (task);
	g_assert (iris_task_is_cancelled (task));
	g_assert (iris_task_is_cancelled (task2));
...
test26 (void)
{
	...
	iris_task_add_dependency (task_after, task);
	iris_task_cancel (task);
	g_assert (iris_task_is_cancelled (task_after));
